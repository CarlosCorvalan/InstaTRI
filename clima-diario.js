...
async function main() {
  ...
  for (const campo of campos) {
    try {
      ...
      console.log(`OK  ${campo.nombre.padEnd(16)} ${fecha}  tmax=${tmax} tmin=${tmin} gdu=${gduDia}`);
      ok++;
    } catch (e) {
      console.error(`ERROR ${campo.nombre}: ${e.message}`);
  ...
  if (errores.length > 0) {
    console.log("Fallaron:", errores.map((e) => e.campo).join(", "));
